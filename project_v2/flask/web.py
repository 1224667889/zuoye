...
from wtforms import (DateField, DateTimeField, FileField, PasswordField,
...
def changedata(id):
    if session['username']==User.query.get(id).name:
        user=User.query.get(id)
        form=DataForm(sex=user.usermessage.sex,birthday=user.usermessage.birthday,signature=user.usermessage.signature)
        if form.validate_on_submit():
            #生成并添加用户数据
            user=User.query.get(id)
